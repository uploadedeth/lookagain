...
const validateApiKey = (): string => {
    ...
    if (!apiKey) {
        throw new Error('API key not found. Please set VITE_GEMINI_API_KEY in your environment variables. Get your API key from: https://aistudio.google.com/app/apikey');
    }
    return apiKey;
};
 ...
 */
export const planDifferences = async (originalPrompt: string, count: number): Promise<string[]> => {
    try {
        const apiKey = validateApiKey();
        const ai = new GoogleGenAI({ apiKey });
...
export const generateModifiedImage = async (
    ...
    try {
        ...
        const originalImagePart = await fileToPart(originalImageFile);
